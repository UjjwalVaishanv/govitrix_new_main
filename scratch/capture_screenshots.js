...
async function capture() {
  ...
  for (const p of projects) {
    console.log(`Navigating to ${p.name || p.slug}: ${p.url}`);
    const page = await context.newPage();
    try {
      await page.goto(p.url, { waitUntil: "networkidle", timeout: 30000 });
      await page.waitForTimeout(4000); // Wait for animations & initial render

      // Take main hero screenshot
      ...
      // Scroll down a bit to capture second screen view
